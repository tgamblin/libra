...
# Mappings from handle array parameter positions in their MPI calls to the positions of their sizes
mpi_array_param_sizes = {
    ("MPI_Testsome", 1) : 0, 
    ("MPI_Testsome", 4) : 3, 
    ("MPI_Testall", 1) : 0, 
    ("MPI_Testall", 3) : 0,
    ("MPI_Type_struct", 3) : 0, 
    ("MPI_Type_get_contents", 6) : 3, 
    ("MPI_Testany", 1) : 0, 
...
class Param:
    ...
    """
    def __init__(self, type, pointers, name, array):
        ...
        if begin and not exclude_re.search(line):
            ...
            for arg in arg_list:
                if arg == '...':   # Special case for Pcontrol.
                    formals.append(Param(None, None, '...', None))
                else:
                    ...
                    formals.append(Param(type.strip(), pointers, name, array))
                arg_num += 1
...
class FortranDelegation:
    """Class for construction delegation to a Fortran wrapper function.  Provides
        ...
        if not (arg.pointers or arg.array):
            ...
            if not arg.isHandle():
                ...
                if arg.pointers:
                    ...
                    # Figure out which param has the count and make an expression for the count
                    count_arg = decl.args[mpi_array_param_sizes[(decl.name, pos)]]
                    count = "*(%s)" % count_arg.name
